...
public class EditorActivity extends AppCompatActivity {
    //mendefinisikan variable uang akan dipakai
    ...
    protected void onCreate(Bundle savedInstanceState) {
        ...
        btnSave.setOnClickListener(view -> {
            if (editName.getText().length()>0 && editEmail.getText().length()>0){
                //Mengambil metjod save data
                saveData(editName.getText().toString(), editEmail.getText().toString());
            }else{
                Toast.makeText(getApplicationContext(), "Silahkab isi semua data !", Toast.LENGTH_SHORT).show();
            }
        });
    ...
    private void saveData(String name, String email){
        ...
        if(id!=null){
            //Kode untuk edit data firestore denbgan ,mengambil id
            ...
            //kode untuk membagikan data dengan .add 
            db.collection("users")
                    .add(user)
